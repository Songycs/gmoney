...
class MapItem extends React.Component {
  ...
  render() {
    ...
    switch(item.type){
      case 1://확대축소
        return (
          <div onClick={item.disable ? null : this.props.onClick} className={`map-button ${mobile}`} >
            <div className={`enlargement ${mobile}`}>+</div>
            <div className={`reduction ${mobile}`}>-</div>
          </div>
        )
      case 2://현위치
        return (
          <div onClick={item.disable ? null : this.props.onClick} className={`map-loc ${mobile}`} >
